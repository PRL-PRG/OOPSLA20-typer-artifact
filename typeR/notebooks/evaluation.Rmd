...
Note: At the top of this file, you'll find the `data_dir` parameter.
Setting `data_dir` to `~/typeR/data-corpus` will yield the results found in the paper.
Setting `data_dir` to `~/typeR/data` will allow you to look at the results of your local run of the analysis.
...
source(file.path(params$scripts_dir, "data-analysis-lib.R"))
...
type_analysis_data <- read_csv(file.path(params$data_dir, "type-analysis", "merged.csv"))
...
if (params$data_dir != "~/typeR/data-corpus") {
  big_simplified_assertion_df <- read_csv(file.path(params$data_dir, "assertions-all.csv"))
  big_simplified_assertion_df %>% group_by(package_name, function_name, parameter_position, actual_type, expected_type) %>% summarize(count = n(), succ_ass = sum(assertion_status)) %>% ungroup -> big_simplified_assertion_df_extra_simpl 
} else {
  # If we're working with the full data, the file has been preprocessed to be smaller so that it can be loaded.
  big_simplified_assertion_df_extra_simpl <- read_csv(file.path(params$data_dir, "assertions-all.csv"))
}

# The output of running propagatr, reduced the data (processed), and combined it into mega CSV.
core_corpus_run <- read_csv(file.path(params$data_dir, "type-traces.csv"), col_types=paste0("cccccl", paste(rep("ccc", 21), collapse=""), "i")) %>% filter(has_dots != "has_dots")
...
# big_simplified_assertion_df_extra_simpl %>% ungroup %>% group_by(package_name, function_name) %>% summarize(count = sum(count))
# UNCOMMENT when paths fixed
assertions_by_fun <- big_simplified_assertion_df_extra_simpl %>% ungroup %>% group_by(package_name, function_name) %>% summarize(count = sum(count))

# For Table 4
df_failed_assertions <- read_csv(file.path(params$data_dir, "assertions-failed.csv"))
# UNCOMMENT when paths are fixed
